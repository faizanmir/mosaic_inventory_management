...
class ItemDetailsViewModel extends BaseViewModel<ItemDetailsNavigator> {
  ...
  void changeCount(int itemCategoryId, int id, double count) async {
    try {
      final response = await _itemDetailsRepo.updateItem(
          itemCategoryId, id, Item.fromMap({"count": count}));
      getNavigator().showMessage(response.message!);
      getNavigator().refresh();
    } catch (e) {
      debugPrint(e.toString());
  ...
  void deleteFile(int itemId, int fileId) async {
    try {
      final response =
          await _itemDetailsRepo.deleteFileFromItem(itemId, fileId);
      getNavigator().showMessage(response.message!);
